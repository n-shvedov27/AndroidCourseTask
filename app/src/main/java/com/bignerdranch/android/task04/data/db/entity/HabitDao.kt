...
interface HabitDao {
    ...
    fun getAllByType(habitType: HabitType, prefix: String): List<Habit>

    @Query("SELECT * FROM habit WHERE type = :habitType AND name LIKE :prefix ORDER BY priority ASC")
    @TypeConverters(HabitTypeConverter::class)
    fun getAllByTypeSortByAscPriority(habitType: HabitType, prefix: String): List<Habit>

    @Query("SELECT * FROM habit WHERE type = :habitType AND name LIKE :prefix ORDER BY priority DESC")
    @TypeConverters(HabitTypeConverter::class)
    fun getAllByTypeSortByDescPriority(habitType: HabitType, prefix: String): List<Habit>
